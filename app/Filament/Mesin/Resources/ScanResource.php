...
use App\Filament\Mesin\Resources\ScanResource\Pages;
use App\Models\Scan;
use App\Models\Ims;
use App\Models\Mjs;
use Filament\Forms;
            ...
            ->schema([
                TextInput::make('qr_token')
                    ...
                    ->afterStateUpdated(function ($state, callable $set) {
                        if ($state) {
                            // Cari di tabel IMS dengan role mesin saja
                            ...
                            // Cari di tabel MJS dengan role mesin saja
                            ...
                            // Jika tidak ditemukan atau bukan role mesin
                            $set('scanned_type', null);
                            $set('scanned_id', null);
                            $set('nama_pos', 'AKSES DITOLAK: QR Code ini bukan untuk role mesin!');
                            $set('nomor_urut', null);
                            $set('status', 'invalid');
                ...
                Select::make('status')
                    ...
                    ->rules([
                        'required',
                        'in:valid', // Hanya boleh valid untuk panel mesin
                        function ($get) {
                            return function (string $attribute, $value, \Closure $fail) use ($get) {
                                if ($value === 'invalid') {
                                    $fail('QR Code tidak valid untuk role mesin. Scan tidak dapat dilanjutkan.');
                                }
                                
                                // Double check QR token
                                $qrToken = $get('qr_token');
                                if ($qrToken) {
                                    $validIms = \App\Models\Ims::where('qr_token', $qrToken)->where('role_type', 'mesin')->exists();
                                    $validMjs = \App\Models\Mjs::where('qr_token', $qrToken)->where('role_type', 'mesin')->exists();
                                    
                                    if (!$validIms && !$validMjs) {
                                        $fail('AKSES DITOLAK: QR Code ini bukan milik role mesin.');
                                    }
                                }
                            };
                        },
