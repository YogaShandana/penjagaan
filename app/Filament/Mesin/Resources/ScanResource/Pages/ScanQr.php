...
class ScanQr extends Page
{
    ...
    protected static string $view = 'filament.mesin.resources.scan-resource.pages.scan-qr';
    ...
    public function processQrScan()
    {
        ...
        // SECURITY CHECK: Pastikan hanya QR role mesin yang bisa diproses
        $isValidForMesin = false;
        if ($this->type === 'ims') {
            $isValidForMesin = Ims::where('qr_token', $cleanToken)->where('role_type', 'mesin')->exists();
        } elseif ($this->type === 'mjs') {
            $isValidForMesin = Mjs::where('qr_token', $cleanToken)->where('role_type', 'mesin')->exists();
        }
        
        if (!$isValidForMesin) {
            // Security block - QR bukan role mesin
            ...
            $this->dispatch('scan-invalid-type', 'QR Code tidak sesuai untuk area mesin');
    ...
    private function scanIms($cleanToken)
    {
        ...
        // Cari di tabel IMS (yang benar) - HANYA ROLE MESIN!
    ...
    private function scanMjs($cleanToken)
    {
        ...
        // Cari di tabel MJS (yang benar) - HANYA ROLE MESIN!
