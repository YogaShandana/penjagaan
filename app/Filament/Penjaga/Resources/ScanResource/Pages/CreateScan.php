...
class CreateScan extends CreateRecord
{
    ...
    {
        // SECURITY CHECK - Pastikan hanya QR penjaga yang bisa discan
        if (isset($data['qr_token'])) {
            ...
            if (!$validIms && !$validMjs) {
                ...
                Log::warning('SECURITY: Unauthorized scan attempt in Penjaga panel', [
        ...
        if (isset($data['status']) && $data['status'] === 'invalid') {
            Notification::make()
                ...
                ->body('QR Code tidak dapat diproses. Pastikan menggunakan QR Code yang benar untuk Panel Penjaga.')
                ->warning()
                ->duration(5000)
