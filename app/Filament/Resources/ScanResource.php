...
class ScanResource extends Resource
{
    ...
    protected static bool $shouldRegisterNavigation = false;  // Hide dari navigation
            ...
            ->schema([
                TextInput::make('qr_token')
                    ...
                    ->afterStateUpdated(function ($state, callable $set) {
                        if ($state) {
                            // Cari di tabel IMS (admin bisa scan semua role)
                            $ims = Ims::where('qr_token', $state)->first();
                            if ($ims) {
                                $set('scanned_type', 'ims');
                                $set('scanned_id', $ims->id);
                                $set('nama_pos', $ims->nama_post . ' (Role: ' . ucfirst($ims->role_type) . ')');
                                $set('nomor_urut', $ims->nomor_urut);
                                $set('status', 'valid');
                                return;
                            }
                            
                            // Cari di tabel MJS (admin bisa scan semua role)
                            $mjs = Mjs::where('qr_token', $state)->first();
                            if ($mjs) {
                                $set('scanned_type', 'mjs');
                                $set('scanned_id', $mjs->id);
                                $set('nama_pos', $mjs->nama_post . ' (Role: ' . ucfirst($mjs->role_type) . ')');
                            ...
                            $set('scanned_id', null);
                            $set('nama_pos', 'QR Code tidak ditemukan');
                            $set('nomor_urut', null);
                            $set('status', 'invalid');
                        }
                    }),
