...
return new class extends Migration
{
    ...
    {
        Schema::table('mjs', function (Blueprint $table) {
            // Check if nomor_urut unique constraint exists before dropping
            $nomorUrutExists = \DB::select(
                "SHOW INDEX FROM mjs WHERE Key_name = 'mjs_nomor_urut_unique'"
            );
            ...
            $compositeExists = \DB::select(
                "SHOW INDEX FROM mjs WHERE Key_name = 'mjs_nomor_urut_role_unique'"
            );
            
            // Tambah composite unique constraint untuk nomor_urut + role_type if not exists
            if (empty($compositeExists)) {
                $table->unique(['nomor_urut', 'role_type'], 'mjs_nomor_urut_role_unique');
            }
        });
    }
    ...
    public function down(): void
    {
        ...
        Schema::table('mjs', function (Blueprint $table) use ($constraintExists) {
            // Drop composite unique constraint if it exists
            if (!empty($constraintExists)) {
                $table->dropUnique('mjs_nomor_urut_role_unique');
            }
