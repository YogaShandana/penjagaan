<x-filament-panels::page class="scan-qr-page" data-component="scan-qr">
    ...
    <script>
        ...
        const SCAN_COOLDOWN = 5000; // 5 detik cooldown untuk mencegah spam
        const ERROR_COOLDOWN = 8000; // 8 detik cooldown untuk error
        let scanProcessing = false; // Flag untuk mencegah multiple processing
        let errorCooldown = false; // Flag untuk mencegah spam error
        ...
        function startSimpleCamera() {
            ...
            navigator.mediaDevices.getUserMedia({
                video: {
                    width: { ideal: 640 },
                    height: { ideal: 480 },
                    facingMode: 'environment' // Use back camera untuk mobile devices
                }
            })
            .then(function(stream) {
        ...
        function captureAndScan() {
            ...
            // QR scanner ready - manual start required
            addDebugInfo('QR scanner ready. Click "Buka Kamera" to start.');
        }
            ...
            try {
                ...
                html5QrCode.start(
                    ...
                    (decodedText, decodedResult) => {
                        ...
                        // ENHANCED: Stronger spam prevention
                        const currentTime = Date.now();
                        
                        // Cek error cooldown terlebih dahulu
                        if (errorCooldown) {
                            addDebugInfo('Error cooldown active - scan ignored');
                            return;
                        }
                        
                        // Cek processing dan duplicate scan
                        if (scanProcessing || (decodedText === lastScannedToken && (currentTime - lastScanTime) < SCAN_COOLDOWN)) {
                            addDebugInfo('Duplicate scan ignored (processing or cooldown active)');
                            return;
                        }
                        
                        // FORCED STOP: Langsung stop scanner sebelum processing
                        if (html5QrCode && isScanning) {
                            try {
                                html5QrCode.stop().then(() => {
                                    addDebugInfo('Scanner force stopped before processing');
                                    html5QrCode = null;
                                }).catch(err => {
                                    addDebugInfo('Error force stopping: ' + err.message);
                                    html5QrCode = null;
                                });
                                isScanning = false;
                            } catch (err) {
                                addDebugInfo('Exception in force stop: ' + err.message);
                                isScanning = false;
                                html5QrCode = null;
                            }
                        }
                        ...
                        updateStatus('🔍 Memverifikasi QR Code...', 'info');
                        
                        // Single Livewire call approach - simplified
                        try {
                            const scanPageElement = document.querySelector('.scan-qr-page[wire\\:id]') || 
                                                   document.querySelector('[data-component="scan-qr"][wire\\:id]') ||
                            ...
                            if (scanPageElement && window.Livewire && window.Livewire.find) {
                                ...
                                if (wireId) {
                                    ...
                                    if (component && typeof component.set === 'function' && typeof component.call === 'function') {
                                        // Set up event listener for server response BEFORE sending request
                                        const handleScanResult = (event) => {
                                            addDebugInfo('Server response received');
                                            
                                            // Stop scanner after server response
                                            if (html5QrCode && isScanning) {
                                                try {
                                                    html5QrCode.stop().then(() => {
                                                        addDebugInfo('QR Scanner stopped after server response');
                                                        html5QrCode = null;
                                                    }).catch(err => {
                                                        addDebugInfo('Error stopping scanner: ' + err.message);
                                                    });
                                                    isScanning = false;
                                                } catch (err) {
                                                    addDebugInfo('Error in scanner stop: ' + err.message);
                                                }
                                            }
                                            ...
                                            window.removeEventListener('livewire:request-finished', handleScanResult);
                                        };
                    
                    // Add event listener for scan errors
                    window.addEventListener('scan-invalid-type', (event) => {
                        addDebugInfo('Received scan-invalid-type event: ' + event.detail);
                        updateStatus('❌ ' + event.detail, 'error');
                        scanProcessing = false;
                    });
                    
                    window.addEventListener('scan-wrong-type', (event) => {
                        addDebugInfo('Received scan-wrong-type event: ' + event.detail);
                        updateStatus('❌ ' + event.detail, 'error');
                        scanProcessing = false;
                    });
                    
                    // Add Livewire event listeners for scan errors
                    document.addEventListener('livewire:dispatch', (event) => {
                        if (event.detail.type === 'scan-invalid-type' || event.detail.type === 'scan-wrong-type') {
                            addDebugInfo('Livewire error: ' + event.detail.type + ' - ' + event.detail.data);
                            
                            // Activate error cooldown to prevent spam
                            errorCooldown = true;
                            scanProcessing = false;
                            
                            updateStatus('❌ ' + event.detail.data, 'error');
                            
                            // Reset error cooldown after delay
                            setTimeout(() => {
                                errorCooldown = false;
                                updateStatus('📷 Siap untuk scan QR berikutnya', 'info');
                                addDebugInfo('Error cooldown reset - ready for next scan');
                            }, ERROR_COOLDOWN);
                        }
                    });
                                        ...
                                        addDebugInfo('Successfully sent to Livewire component: ' + wireId);
                                        
                                        // Backup timeout in case event doesn't fire
                                        setTimeout(() => {
                                            if (scanProcessing) {
                                                addDebugInfo('Backup timeout triggered - stopping scanner');
                                                handleScanResult();
                                            }
                                        }, 5000);
                                        
                                    } else {
                                        addDebugInfo('Component found but methods not available');
                                        scanProcessing = false;
                                    }
                                } else {
                                    addDebugInfo('No wire:id found on scan page element');
                                    scanProcessing = false;
                                }
                            } else {
                                addDebugInfo('No suitable scan page element found');
                                scanProcessing = false;
                            }
                        } catch (err) {
                            addDebugInfo('Livewire communication error: ' + err.message);
                            console.error('Livewire error details:', err);
                            scanProcessing = false;
                        }
                        setTimeout(() => {
                            scanProcessing = false;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            addDebugInfo('Page loaded - QR scanner ready for manual start');
            
            setTimeout(() => {
                // Show debug and status by default
                document.getElementById('scan-status').style.display = 'block';
                
                // Manual start only - no auto camera
                updateStatus('📷 Klik "Buka Kamera" untuk memulai scan QR', 'info');
            }, 1000);
        });
